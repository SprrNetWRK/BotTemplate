...
class clipGet extends Command {
    ...
    async run(message, args) {
        try {
            const clientId = this.client.guildconfig.TwitchClientId;
            const authProvider = new ClientCredentialsAuthProvider(clientId, this.client.guildconfig.TwitchclientSecret);
            ...
            if (method === "id") {
                ...
                const clip = await apiClient.helix.clips.getClipById(id);
                ...
                const clip = await apiClient.helix.clips.getClipsForBroadcaster(user._data.id, {
                    limit: num,
                });
                let text = "";
...
• ${message.language.get("CLIPGET_STREAMER_EMBED_CREATEDBY")} ${clip.data[i]._data.creator_name} ${message.language.get("CLIPGET_STREAMER_EMBED_CREATEDAT", clip.data[i]._data.created_at)}`
                ...
                const game = await apiClient.helix.games.getGameByName(gameToSearch);
                if (game === null) return message.channel.send(message.language.get("CLIPGET_GAME_NOT_FOUND"));
                num = args[2];
                if (isNaN(parseInt(num))) num = 1;
                if (parseInt(num) > 10 || parseInt(num) < 1) return message.channel.send(message.language.get("CLIPGET_STREAMER_NUMBER"));
                const clip = await apiClient.helix.clips.getClipsForGame(game._data.id, {
                    limit: num,
                });
                let text = "";
                for (let i = 0; i < clip.data.length; i++) {
