...
class Config extends Command {
    ...
    async run(message) {
        try {
            const authProvider = new ClientCredentialsAuthProvider(message.guildconfig.TwitchClientId, message.guildconfig.TwitchclientSecret);
		...
		WHERE guild_id="${message.guild.id}"`;
            ...
            mysqlcon.query(sql, async function (err, result, fields) {
                ...
                if (stream_ar === "") {
                    ...
                    var user = await apiClient.helix.users.getUserById(stream_ar);
                    ...
                    for (var i = 0; i < stream_ar.split("/").length; i++) {
                        var user = await apiClient.helix.users.getUserById(ar_ids[i]);
                        if (text === "") {
                            text = `**•** ${user._data.display_name} (${user._data.login} - ${user._data.id})`;
                        } else {
